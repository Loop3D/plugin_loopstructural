...
class MockFault:
    ...
    def __init__(self, name="TestFault", normal_vector=None):
        ...
        self.fault_normal_vector = (
            normal_vector if normal_vector is not None else np.array([1.0, 0.0, 0.0])
        )
...
class TestFaultDipRetrieval(unittest.TestCase):
    ...
    def test_dip_from_stored_data(self):
        ...
        fault_data = pd.DataFrame(
            {'X': [0, 1, 2], 'Y': [0, 1, 2], 'Z': [0, 0, 0], 'dip': [45, 45, 45]}
        )
    ...
    def test_dip_fallback_to_normal_vector(self):
        ...
        # No stored dip data
        fault_data = pd.DataFrame({'X': [0, 1, 2], 'Y': [0, 1, 2], 'Z': [0, 0, 0]})
    ...
    def test_pitch_from_stored_data(self):
        ...
        fault_data = pd.DataFrame(
            {'X': [0, 1, 2], 'Y': [0, 1, 2], 'Z': [0, 0, 0], 'pitch': [30, 30, 30]}
        )
